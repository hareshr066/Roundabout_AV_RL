...
def run_single_episode_success():
    model_path = "results/models/agent_spatial_curriculum_30k.zip"
    
    print("=" * 80)
    print("        RUNNING ROUNDABOUT AV SIMULATION ONCE (STAGE A SUCCESS RUN)")
    print("=" * 80)
    ...
    print("\nInitializing Headless SUMO Environment (Spawn Distance: 15m)...")
    env = RoundaboutEnv(
        gui=False,
    ...
    try:
        ...
        while not done:
            ...
            print(f"Step {steps:3d} | Speed: {obs[0]:5.2f} m/s | Dist to Entry: {obs[1]:6.2f} m | Action Accel: {action[0]:+5.2f} m/s^2 | Reward: {reward:+6.2f}")
